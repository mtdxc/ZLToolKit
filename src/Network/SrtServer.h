...
#ifndef SrtServer_SrtServer_H
#define SrtServer_SrtServer_H
...
class SrtServer : public Server {
public:
    using Ptr = std::shared_ptr<SrtServer>;

    /**
     * 创建tcp服务器，listen fd的accept事件会加入到所有的poller线程中监听
     * 在调用SrtServer::start函数时，内部会创建多个子SrtServer对象，
     * 这些子SrtServer对象通过Socket对象克隆的方式在多个poller线程中监听同一个listen fd
     * 这样这个TCP服务器将会通过抢占式accept的方式把客户端均匀的分布到不同的poller线程
     * 通过该方式能实现客户端负载均衡以及提高连接接收速度
     */
    explicit SrtServer(const EventPoller::Ptr &poller = nullptr);
    ~SrtServer() override;
    ...
    */
    ...
    void start(uint16_t port, const std::string &host = "::", uint32_t backlog = 1024) {
        ...
        _session_alloc = [](const SrtServer::Ptr &server, const Socket::Ptr &sock) {
...
private:
    ...
    std::function<SessionHelper::Ptr(const SrtServer::Ptr &server, const Socket::Ptr &)> _session_alloc;

    //对象个数统计
    ObjectStatistic<SrtServer> _statistic;
};

} /* namespace toolkit */
#endif /* SrtServer_SrtServer_H */
