...
StatisticImp(SrtClient)

SrtClient::SrtClient(const EventPoller::Ptr &poller) : SocketHelper(nullptr) {
...
void SrtClient::shutdown(const SockException &ex) {
    _timer.reset();
    SocketHelper::shutdown(ex);
}

bool SrtClient::alive() const {
    if (_timer) {
        //连接中或已连接
        return true;
    }
...
void SrtClient::setNetAdapter(const string &local_ip) {
    _net_adapter = local_ip;
}

void SrtClient::startConnect(const string &url, uint16_t port, float timeout_sec, uint16_t local_port) {
    weak_ptr<SrtClient> weak_self = shared_from_this();
    ...
    }, timeout_sec, _net_adapter, local_port, true);
}

void SrtClient::onSockConnect(const SockException &ex) {
    if (ex) {
        //连接失败
        _timer.reset();
        onConnect(ex);
